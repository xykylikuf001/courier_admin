...
import type {PlaceVisible, PlaceCreateWithTranslation, ValidationError} from "@/openapi/client";
...
interface IResponse {
    ...
    data: PlaceVisible | null;
}

export const create = async (values: PlaceCreateWithTranslation): Promise<IResponse> => {
    ...
    try {
        const response = await fetchClient.location.placeCreate(
            {requestBody: values}
        );
