...
import type {LobbyVisible, LobbyCreate, ValidationError} from "@/openapi/client";
...
export const create = async (values: LobbyCreate): Promise<IResponse> => {
    ...
    try {
        const response = await fetchClient.lobby.lobbyCreate({requestBody: values});
