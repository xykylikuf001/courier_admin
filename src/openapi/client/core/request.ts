...
const catchErrorCodes = async (config: OpenAPIConfig, options: ApiRequestOptions, result: ApiResult): Promise<void> => {
    ...
    if (result.status == 404 && error) {
        if (config.CALLBACKS){
            const callback = config.CALLBACKS[result.status]
...
export const request = <T>(config: OpenAPIConfig, options: ApiRequestOptions): CancelablePromise<T> => {
    return new CancelablePromise(async (resolve, reject, onCancel) => {
        ...
        try {
            ...
            if (!onCancel.isCancelled) {
                const response = await sendRequest(
                    config, options, url, body, formData, headers, onCancel, next
                );
        ...
        if (tryError instanceof ApiError) {
            if (config.CALLBACKS){
                const callback = config.CALLBACKS[tryError.status]
                // console.log(config.CALLBACKS, config.CALLBACKS[error.status])
                if (callback) {
                    console.log(callback, "API error")
                    await callback()
                }
            }
        }
